...
from utils import build_logger, ErrorCode
...
class Controller:
    ...
    def register_daemon(
        self, daemon_name: str, daemon_IP: str, daemon_port:int
    ):
        if daemon_IP not in self.daemon_info:
            self.daemon_info[daemon_IP]={
                "name":daemon_name,
                "IP":daemon_IP,
                "port":daemon_port
            }
            logger.info(f"Register done: {daemon_name}, {daemon_IP}")
            return True
        else:
    ...
    def list_daemons(self):
        daemons = []
        for i, info in self.daemon_info.items():
            daemon_heartbeat=f"http://{info['IP']}:{info['port']}/heartbeat"
            try:
                res=requests.get(daemon_heartbeat)
                daemons.append({"name":info["name"],"address":info["IP"]})
            except Exception:
                pass
        return daemons
...
async def register_daemon(request: Request):
    ...
    if controller.register_daemon(data["daemon_name"], data["daemon_IP"],data["daemon_port"]):
        return status.HTTP_200_OK
    return status.HTTP_208_ALREADY_REPORTED

@app.post("/list_daemons")
async def list_daemons():
    machines = controller.list_daemons()
...
def create_controller():
    ...
    logger = build_logger("controller", "controller.log", args.log_dir)
